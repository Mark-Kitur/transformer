...
class PositionalEncoding(nn.Module):
    def __init__(self,d_model:int, seq_len:int, dropout:float )->None:
        ...
        pe = pe.unsqueeze(0) #(1,seg_len, d_model)
        self.register_buffer('pe',pe)
    def forward(self,x):
        x= x + (self.pe[:, : x.shape[1], :]).requires_grad_(False) # not to be learnt since they are fixed
...
class LayerNormalization(nn.Module):
    ...
    def forward(self, x):
        ...
        # Ensure alpha and bias are on same device as x
...
class FeedForwardBlock(nn.Module):
    ...
    def forward(self,x):
        #(Batch, seq_len, d_model) --> (Batch_len, d_ff)-->(Batch , seq_len,d_model)
...
class MultiHeadAttentionBlock(nn.Module):
    ...
    def attention(query,key,value, mask, dropout:nn.Dropout):
        ...
        # batch,h,seg_len ,d_k --> batch, h, seq_len, seq_len
